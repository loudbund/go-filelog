...
// 函数1：获取日志操作实例
func New(Folder string, Date string) *uFileLog {
	return &uFileLog{
		folderLog:   Folder,
		date:        Date,
		indexFp:     nil,
...
// 辅助函数1：初始化内容文件句柄
func (Me *uFileLog) initFpData(index int) error {
	// 判断日期目录是否存在,没有将会创建
...
// 辅助函数2：初始化自增id
func (Me *uFileLog) initAutoId() error {
	if err := Me.initFpIndex(); err != nil {
		return err
	}
...
// 辅助函数3：初始化索引文件句柄
func (Me *uFileLog) initFpIndex() error {
	if Me.indexFp == nil {
		// 判断日期目录是否存在
		folderDate, err := Me.getLogDateFolder()
		if err != nil {
...
// 辅助函数4：获取日志文件夹
func (Me *uFileLog) getLogDateFolder() (string, error) {
...
// 辅助函数5：读取指定长度数据
func (Me *uFileLog) fileReadLength(fp *os.File, Length int) ([]byte, error) {
	// 读取一批
...
// 数字类型转换1： int转[]byte
func (Me *uFileLog) int2Bytes(n int) []byte {
	x := int32(n)
...
// 数字类型转换2： int16转[]byte
func (Me *uFileLog) int16ToBytes(i int16) []byte {
	var buf = make([]byte, 2)
...
// 数字类型转换3： int32转[]byte
func (Me *uFileLog) int32ToBytes(i int32) []byte {
	var buf = make([]byte, 4)
...
// 数字类型转换4： int64转[]byte
func (Me *uFileLog) int64ToBytes(i int64) []byte {
	var buf = make([]byte, 8)
...
// 数字类型转换5： []byte转int
func (Me *uFileLog) bytes2Int(b []byte) int {
...
// 数字类型转换6： []byte转int16
func (Me *uFileLog) bytes2Int16(buf []byte) int16 {
	return int16(binary.BigEndian.Uint16(buf))
}

// 数字类型转换7： []byte转int32
func (Me *uFileLog) bytes2Int32(buf []byte) int32 {
	return int32(binary.BigEndian.Uint32(buf))
}

// 数字类型转换8： []byte转int64
func (Me *uFileLog) bytes2Int64(buf []byte) int64 {
